...
describe('GET /api/committee-inspections/[workPeriodId]', () => {
...
describe('POST /api/committee-inspections/[workPeriodId]', () => {
  it(
    'returns 400 when inspectors array is empty',
    async () => {
      const wpId = await createWorkPeriod();
      cookieCtx.userId = PM_USER_ID;
  ...
  it(
    'creates a committee inspection record',
    async () => {
      const wpId = await createWorkPeriod();
      cookieCtx.userId = PM_USER_ID;
      ...
      const created = (await res.json()) as {
        data: { id: string; result: string; inspectors: string[] };
      };
      expect(created.data.result).toBe('pass');
      expect(created.data.inspectors).toEqual(['user-002', 'user-004']);

      const list = await GET(new Request('http://localhost/'), {
        params: { workPeriodId: wpId },
      });
      const body = (await list.json()) as { data: Array<{ id: string }> };
      expect(body.data.some((c) => c.id === created.data.id)).toBe(true);
    },
    SLOW_DB_TIMEOUT,
  );
});
